...
Installs Nginx on Ubuntu server.

This role installs and configures the latest version of Nginx from the Nginx apt repository. Allows to define Virtual hosts via Ansible variables (standard or custom template).
Configuration part of the role can be used in Application configuration scenarios and startup scripts.
...
The user under which Nginx will run. Defaults to `www-data` for Ubuntu.
...
Allows you to set a different repository for the installation of Nginx (e.g. target release). Ansible will use that as the `-t` option while installing Nginx.

    nginx_ppa_use: false
    nginx_ppa_version: stable

Allows you to use the official Nginx PPA instead of the system's package. You can set the version to `stable` or `development`.
...
Create the child template in the path you configured above and extend stock `nginx` template file relative to your `playbook.yml`.
...
Author Information
------------------
This role originally was created in 2014 by Jeff Geerling and rewritten by Serge for them needs.
